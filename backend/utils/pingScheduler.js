import ping from 'ping';
import {
  initDb
} from '../db/index.js';

export async function startPingScheduler(intervalMs = 1000) {
  const db = await initDb();

  async function pingAllDevices() {
    const devices = await db.all(`SELECT id, ip, status, uptime_seconds FROM devices`);

    await Promise.all(devices.map(async (device) => {
      try {
        const result = await ping.promise.probe(device.ip, {
          timeout: 1
        });

        const isOnline = result.alive;
        const now = new Date().toISOString();

        if (isOnline) {
          const newUptime = device.status ? device.uptime_seconds + intervalMs / 1000 : 0;
          await db.run(`
        UPDATE devices
        SET status = 1, last_seen = ?, uptime_seconds = ?
        WHERE id = ?
      `, [now, newUptime, device.id]);
        } else {
          await db.run(`
        UPDATE devices
        SET status = 0, uptime_seconds = 0
        WHERE id = ?
      `, [device.id]);
        }

        // console.log(`${device.ip} → ${isOnline ? '🟢 Online' : '🔴 Offline'}`);
      } catch (err) {
        console.error(`❌ Failed to ping ${device.ip}`, err);
      }
    }));
  }

  setInterval(pingAllDevices, intervalMs);
  console.log(`✅ Ping scheduler with uptime tracking started (every ${intervalMs / 1000}s)`);
}